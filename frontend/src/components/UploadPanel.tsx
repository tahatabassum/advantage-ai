...
const UploadPanel: React.FC<UploadPanelProps> = ({ onAnalyze, isLoading }) => {
    ...
    <div className="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden">
      <div className="p-8 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 font-medium mt-1">Upload up to 5 creatives for comparison or bulk audit.</p>
        </div>
        {files.length > 0 && (
           <div className="px-3 py-1 bg-blue-100 text-blue-700 rounded-lg text-[10px] font-black uppercase tracking-widest">
            {files.length} Variant{files.length > 1 ? 's' : ''} Selected
          </div>
        )}
      </div>
